...
module.exports = function(app) {
    ...
    var socketsender = {
        emit: function(userid, msgtype, msg) {
            ...
            console.log("[emit] pid : " + process.pid + ', userid: ' + userid);
            ...
            redis_client.get('taracot_socketio_sessions_' + sid, function(err, _sessions) {
                ...
                for (var i = 0; i < sessions.length; i++) {
                    if (io.sockets.connected[sessions[i]]) {
                    	console.log('[emit] sending to session: ' + sessions[i]);
                        io.sockets.connected[sessions[i]].emit(msgtype, msg);
                    }
                }
            });
        }
