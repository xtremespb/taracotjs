...
var dragMaster = (function() {
	...
	function mouseMove(e) {
		e = fixEvent(e);
		if (mouseDownAt) {
			if (Math.abs(mouseDownAt.x - e.pageX) < 5 && Math.abs(mouseDownAt.y - e.pageY) < 5)	return false;
			// Start drag
...
function DragObject(element) {
	...
	this.onDragStart = function(offset) {
		$('#files_grid').click();
		var nodes = getByClass('taracot-warehouse-image-thumbnail'),
			i = -1,
			node;
		if (!getByClass('taracot-warehouse-image-thumbnail-selected').length) element.click();
		if (!element.getAttribute('class').match('taracot-warehouse-image-thumbnail-selected'))	element.setAttribute('class', element.getAttribute('class') + ' taracot-warehouse-image-thumbnail-selected');
		while (node = nodes[++i]) {
	...
	this.onDragMove = function(x, y) {
		var nodes_sel = getByClass('taracot-warehouse-image-thumbnail-selected'),
			i = -1,
			node_sel;
