module.exports = function(app) {
    ...
    router.get('/', function(req, res) {
        ...
        var body = app.get('renderer').render_file(path.join(__dirname, 'views'), 'settings_control', {
            lang: i18nm,
    ...
    router.post('/data/save', function(req, res) {
        ...
        for (var i = 0; i < app.get('config').locales.avail.length; i++) {
            if (olang == app.get('config').locales.avail[i]) {
                _olang = app.get('config').locales.avail[i];
            }
        }
        olang = _olang;
        if (rep.status === 0) {
