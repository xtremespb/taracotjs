...
$('#btn_descitem_add').click(function() {
    ...
    tr += '<td style="text-align:right"><button class="uk-button uk-button-danger taracot-warehouse_conf-descitem-delete"><i class="uk-icon-trash-o"></i></button></td></tr>';
    $('#warehouse_conf_descitems_tb').append(tr);
    $('.taracot-warehouse_conf-descitem-delete').unbind();
    $('.taracot-warehouse_conf-descitem-delete').click(btn_taracot_descitem_delete_handler);
    $('#warehouse_conf_descitems').show();
});
...
$('#btn_collection_add').click(function() {
    ...
    tr += '<td style="text-align:right;width:80px"><button class="uk-button taracot-warehouse_conf-collections-add"><i class="uk-icon-plus"></i></button>&nbsp;<button class="uk-button uk-button-danger taracot-warehouse_conf-collections-delete"><i class="uk-icon-trash-o"></i></button></td></tr>';
    $('#warehouse_conf_collections_tb').append(tr);
    $('.taracot-warehouse_conf-collections-delete').unbind();
    $('.taracot-warehouse_conf-collections-delete').click(btn_taracot_collection_delete_handler);
    $('.taracot-warehouse_conf-collections-add').unbind();
    $('.taracot-warehouse_conf-collections-add').click(btn_taracot_collection_add_handler);
    $('#warehouse_conf_collections').show();
});
...
$('#btn_currency_add').click(function() {
    ...
    for (var i = 0; i < locales.length; i++) tr += '<td><input type="text" class="uk-width-1-1"></td>';
    tr += '<td style="width:40px"><button class="uk-button uk-button-danger taracot-warehouse_conf-curs-delete"><i class="uk-icon-trash-o"></i></button>&nbsp;<button class="uk-button taracot-btn-curs-sort"><i class="uk-icon uk-icon-unsorted"></i></button></td></tr>';
    $('#warehouse_conf_curs_tb').append(tr);
    $('.taracot-warehouse_conf-curs-delete').unbind();
    $('.taracot-warehouse_conf-curs-delete').click(btn_taracot_curs_delete_handler);
    $('#warehouse_conf_curs').show();
    UIkit.sortable($('#warehouse_conf_curs_tb'), {
...
$('#btn_ship_add').click(function() {
    ...
    for (var i = 0; i < locales.length; i++) tr += '<td><input type="text" class="uk-width-1-1"></td>';
    tr += '<td style="width:60px" nowrap><button class="uk-button uk-button-danger taracot-warehouse_conf-ship-delete"><i class="uk-icon-trash-o"></i></button>&nbsp;<button class="uk-button taracot-btn-ship-sort"><i class="uk-icon uk-icon-unsorted"></i></button></td></tr>';
    $('#warehouse_conf_ship_tb').append(tr);
    $('.taracot-warehouse_conf-ship-delete').unbind();
    $('.taracot-warehouse_conf-ship-delete').click(btn_taracot_ship_delete_handler);
    $('#warehouse_conf_ship').show();
    UIkit.sortable($('#warehouse_conf_ship_tb'), {
        dragCustomClass: 'uk-form',
...
var btn_taracot_descitem_delete_handler = function() {
    ...
    if ($('#warehouse_conf_descitems tr').length > 1) {
        $('#warehouse_conf_descitems').show();
    } else {
...
var btn_taracot_curs_delete_handler = function() {
    if (confirm(_lang_vars.confirm_delete_descitem)) $(this).parent().parent().remove();
    if ($('#warehouse_conf_curs tr').length > 1) {
        $('#warehouse_conf_curs').show();
    } else {
...
var btn_taracot_collection_delete_handler = function() {
    if (confirm(_lang_vars.confirm_delete_descitem)) $(this).parent().parent().remove();
    if ($('#warehouse_conf_collections tr').length > 1) {
        $('#warehouse_conf_collections').show();
    } else {
...
var btn_taracot_ship_delete_handler = function() {
    if (confirm(_lang_vars.confirm_delete_descitem)) $(this).parent().parent().remove();
    if ($('#warehouse_conf_ship tr').length > 1) {
        $('#warehouse_conf_ship').show();
    } else {
...
$('#btn_save').click(function() {
    ...
    $.ajax({
        type: 'POST',
        url: '/cp/warehouse_conf/config/save',
        data: {
            descitems: descitems,
...
var _get_items_array = function() {
    var descitems = [],
        _cnt = 0,
        _ai = {};
    $('#warehouse_conf_descitems_tb  > tr > td').each(function() {
...
var _get_collections_array = function() {
    var collections = [],
        _cnt = 0,
        _ai = {};
    $('#warehouse_conf_collections_tb  > tr > td').each(function() {
        if (_cnt === 0) _ai.id = $(this).children().first().val();
        if (_cnt === 1) {
            _ai.items = [];
...
var _get_curs_array = function() {
    var curs = [],
        _cnt = 0,
        _ai = {};
    $('#warehouse_conf_curs_tb  > tr > td').each(function() {
        if (_cnt === 0) _ai.id = $(this).children().first().val();
...
var _get_ship_array = function() {
    var ship = [],
        _cnt = 0,
        _ai = {};
    $('#warehouse_conf_ship_tb  > tr > td').each(function() {
        if (_cnt === 0) _ai.id = $(this).children().first().val();
...
var _get_misc_array = function() {
    var misc = [],
        ...
        _ai = {},
        _gc = 0;
    $('#warehouse_conf_misc_tb  > tr > td').each(function() {
        if (_cnt === 0) {
...
$(document).ready(function() {
    $('#warehouse_conf_descitems_tb  > tr').each(function() {
        $(this).remove();
    });
    $('#warehouse_conf_collections_tb  > tr').each(function() {
        $(this).remove();
    });
    ...
    $('#warehouse_conf_collections_tb  > tr > td').each(function(i) {
        if (_cnt === 0) {
