module.exports = function(app) {
    ...
    router.post('/blog/post/save', function(req, res, next) {
        ...
        var post_id = req.body.post_id,
            ...
            post_draft = req.body.post_draft,
        ...
        if (post_keywords) {
            var post_keywords_arr = post_keywords.replace(/[\'\"<>\&]/g, "").split(',');
            var post_keywords_arr_unique = post_keywords_arr.filter(function(item, pos) {
                return post_keywords_arr.indexOf(item) == pos;
            }).sort();
            post_keywords = post_keywords_arr_unique.join(', ');
        }
        ...
        if (post_id) { // Save changes to the old post
            app.get('mongodb').collection('blog').findOne({
