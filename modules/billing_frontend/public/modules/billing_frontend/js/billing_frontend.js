...
$(document).ready(function() {
    ...
    if (!init_accounts.length) {
        $('#taracot_billing_table_hosting > tbody').html('<tr><td colspan="4">' + _lang_vars.no_records + '</td></tr>');
        $('#taracot_billing_table_domains > tbody').html('<tr><td colspan="3">' + _lang_vars.no_records + '</td></tr>');
    } else {
        for (var ai in init_accounts) {
            ...
            if (init_accounts[ai].btype == 'h') $('#taracot_billing_table_hosting > tbody').append('<tr><td>' + init_accounts[ai].baccount + '</td><td style="text-align:center">' + hosting_plan_hash[init_accounts[ai].bplan] + '</td><td style="text-align:center">' + init_accounts[ai].bexp + '</td><td>' + btn_host_up + '</td></tr>');
            if (init_accounts[ai].btype == 'd') $('#taracot_billing_table_domains > tbody').append('<tr><td>' + init_accounts[ai].baccount + '.' + init_accounts[ai].bplan + '</td><td style="text-align:center">' + moment(parseInt(init_accounts[ai].bexp)).format('L') + '</td><td></td></tr>');
        }
    }
